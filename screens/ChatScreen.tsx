...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    if (selectedChannel && user) {
      ...
      const subscription = setupRealtimeSubscription(selectedChannel.id)
  ...
  const getUnreadCount = async (channelId: string): Promise<number> => {
    ...
    try {
      ...
      const { data: channelMessages, error: countError } = await supabase
        .from('chat_messages')
        .select('id')
  ...
  const updateChannelUnreadCount = async (channelId: string) => {
    const unreadCount = await getUnreadCount(channelId)
    setChannels(prev => 
      prev.map(ch => 
